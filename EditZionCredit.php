...
$sql = "SELECT * FROM zioncreditcard WHERE ID='$ID' ";
...
$searchQuery = "SELECT * FROM zioncreditcard WHERE ID = $data[0]";
...
{
	$data = getPosts();
	$update_Query = "UPDATE `zioncreditcard` SET `CreditCard`='$data[1]', `Rate`='$data[2]', `APR`='$data[3]' WHERE `ID` = $data[0]";
	try{
...
{
	$data = getPosts();
	$delete_Query = "DELETE FROM `zioncreditcard` WHERE `ID` = $data[0]";
	try{
...
<body>
      <?php
  echo "<h1>Edit Zion's Credit Cards</h1>";?>
    <h1><?php echo $ID ?></h1>
  
  <form action="EditZionCredit.php?ID={$row['ID']}" method="post" enctype="multipart/form-data">
