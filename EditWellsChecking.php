...
$sql = "SELECT * FROM wellschecking WHERE ID='$ID' ";
...
$searchQuery = "SELECT * FROM wellschecking WHERE ID = $data[0]";
...
{
	$data = getPosts();
	$update_Query = "UPDATE `wellschecking` SET `Checking`='$data[1]' WHERE `ID` = $data[0]";
	try{
...
{
	$data = getPosts();
	$delete_Query = "DELETE FROM `wellschecking` WHERE `ID` = $data[0]";
	try{
...
<html>
<head>
  <title>Edit Checking Rates</title>
...
<body>
  ...
  <form action="EditWellsChecking.php?ID={$row['ID']}" method="post" enctype="multipart/form-data">
