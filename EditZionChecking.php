...
$sql = "SELECT * FROM zionchecking WHERE ID='$ID' ";
...
$searchQuery = "SELECT * FROM zionchecking WHERE ID = $data[0]";
...
{
	$data = getPosts();
	$update_Query = "UPDATE `zionchecking` SET `Checking`='$data[1]' WHERE `ID` = $data[0]";
	try{
...
{
	$data = getPosts();
	$delete_Query = "DELETE FROM `zionchecking` WHERE `ID` = $data[0]";
	try{
...
<body>
  ...
  <form action="EditZionChecking.php?ID={$row['ID']}" method="post" enctype="multipart/form-data">
